...
public class R implements CommandExecutor {
	...
	public R(MessageObj messageObj) {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		try {
			...
			playersender.sendMessage("§7[" + playersender.getDisplayName() + "§7->" + p.getDisplayName() + "§7]" + msg);
		...
		for (int i = 0, j = messageObj.getSocial().size(); i < j; i++) {
			if (!(messageObj.getSocial().get(i) == playersender.getName()
				...
				player.sendMessage("§6§l[§aSpy§6§l]§7[" + playersender.getDisplayName() + "§7->" + p.getDisplayName()
						+ "§7]" + msg);
			}
		}
